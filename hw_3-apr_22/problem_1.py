...
def get_composite(epsilon):
    def f(t):
        ...
        order_epsilon = (2 - t)*np.exp(-t) - 2*np.exp(-t/epsilon)
...
def main():
    ...
    for epsilon in epsilons:
        ...
        relative_error = (y_actual - y_composite)/y_actual
        first_term_error = abs(y_actual - y_first_term)
        relative_first_term_error = (y_actual - y_first_term)/y_actual
        data[str(epsilon)] = [y_actual, y_composite, error, y_first_term, first_term_error, relative_error, relative_first_term_error]
    ...
    plt.semilogx(x, data[str(epsilons[0])][3], "--", color="0.5", linewidth=2, label=r"$O(1)$ composite expansion")
    for epsilon in epsilons:
    ...
    plt.text(0.85*10**(-1), 0.625, r"$\varepsilon=10^{-1}$", rotation=60)
    plt.legend(loc=4, fontsize=10)
    plt.savefig("figures/semilog.png", filetype="png", dpi=300)
    plt.close()
    garbage.collect()
    ...
    plt.plot(x, data[str(epsilons[0])][3], "--", color="0.5", linewidth=2, label=r"$O(1)$ composite expansion")
    for epsilon in epsilons:
    ...
    plt.text(0.1, 0.68, r"$\varepsilon=10^{-1}$", rotation=50)
    plt.legend(loc=0, fontsize=10)
    plt.savefig("figures/standard.png", filetype="png", dpi=300)
    plt.close()
    garbage.collect()
    ...
    plt.text(10**(-1), 7*10**(-8), r"$\varepsilon=10^{-4}$")
    plt.text(10**(-1), 6*10**(-6), r"$\varepsilon=10^{-3}$")
    plt.text(10**(-1), 8*10**(-4), r"$\varepsilon=10^{-2}$")
    plt.text(10**(-1), 4*10**(-2), r"$\varepsilon=10^{-1}$")
    plt.savefig("figures/errors_epsilon.png", filetype="png", dpi=300)
    plt.close()
    garbage.collect()

    plt.figure()
    for epsilon in epsilons:
        plt.loglog(x, data[str(epsilon)][5], color="k")
        plt.ylim([0, 1.1])
        plt.xlabel(r"$x$")
        plt.ylabel("absolute error")
    plt.title(r"Absolute Error of $O(1)$ Composite Expansion")
    plt.text(10**(-1), 8*10**(-10), r"$\varepsilon=10^{-5}$")
    plt.text(10**(-1), 8*10**(-8), r"$\varepsilon=10^{-4}$")
    plt.text(10**(-1), 8*10**(-6), r"$\varepsilon=10^{-3}$")
    plt.text(10**(-1), 8*10**(-4), r"$\varepsilon=10^{-2}$")
    plt.text(10**(-1), 5*10**(-2), r"$\varepsilon=10^{-1}$")
    plt.savefig("figures/errors_1.png", filetype="png", dpi=300)
    plt.close()
    garbage.collect()

    # plt.figure()
    # for epsilon in epsilons:
    #     plt.semilogy(x, data[str(epsilon)][4], color="k")
    #     plt.ylim([0, 1.1])
    #     plt.xlabel(r"$x$")
    #     plt.ylabel("relative error")
    # plt.title(r"Relative Error of $O(\varepsilon)$ Composite Expansion")
    # plt.show()
    # plt.close()
